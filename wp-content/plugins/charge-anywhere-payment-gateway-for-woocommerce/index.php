...
function woocommerce_tech_chargeanywhere_init() {
   ...
   */
   ...
   /** 
   * Charge Any Where Payment Gateway class
   */
   class WC_ChargeAnyWhere_SIP extends WC_Payment_Gateway 
   {
      protected $msg = array();
      ...
      public function __construct(){
         ...
         $this->msg['message']   = "";
         $this->msg['class']     = "";
         $this->newrelay         = home_url(). '/wc-api/WC_ChargeAnyWhere_SIP';
         ...
         add_action( 'woocommerce_api_wc_chargeanywhere_sip' , array( $this, 'check_authorize_response' ) );
   ...
   **/
   function payment_fields() { 
      ...
      wp_register_script( 'wc-chargeanywhere-script', plugin_dir_url( __FILE__ ) . 'assets/chargeanywhere-frontend.js', array('jquery','jquery-payment'), '1.0', true );
	...
	public function post_transaction( $payload ) {
      ...
      **/
      function process_payment($order_id){
         ...
         $card_raw       = isset( $_POST['chargeanywhere-card-number'] ) ? sanitize_text_field( wp_unslash( $_POST['chargeanywhere-card-number'] ) ) : '';
         $card_number    = str_replace( ' ', '', $card_raw );
         $exp_raw        = isset( $_POST['chargeanywhere-card-expiry'] ) ? sanitize_text_field( wp_unslash( $_POST['chargeanywhere-card-expiry'] ) ) : '';
         ...
         $cvc            = isset( $_POST['chargeanywhere-card-cvc'] ) ? sanitize_text_field( wp_unslash( $_POST['chargeanywhere-card-cvc'] ) ) : '';
         ...
         $account_number      = isset( $_POST['chargeanywhere-account-number'] ) ? sanitize_text_field( wp_unslash( $_POST['chargeanywhere-account-number'] ) ) : '';
         $account_number      = str_replace( ' ', '', $account_number );
         $routing_number      = isset( $_POST['chargeanywhere-routing-number'] ) ? sanitize_text_field( wp_unslash( $_POST['chargeanywhere-routing-number'] ) ) : '';
         $routing_number      = str_replace( ' ', '', $routing_number );
         $acc_type            = isset( $_POST['chargeanywhere-acctype'] ) ? sanitize_text_field( wp_unslash( $_POST['chargeanywhere-acctype'] ) ) : '';
   ...
   function custom_script_in_admin() {
      wp_register_script( 'custom_handle_name',plugin_dir_url( __FILE__ ) . 'assets/chargeanywhere.js', '',true );
